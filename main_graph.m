...
%% SOLVE PROBLEM
sol_noNoise = [];
for t=1:length(unique(Release))
    idx = unique(Release); % Find the events (i.e. release of products)
    % Consider the jobs currently present in the shopfloor
    S0 = Release(Release <= idx(t));
    ...
    for i=1:length(BigOmega)
        u=1;
        vec_gamma = [];
        sol_noNoise = Graph_minimization(G_init,G_j,P, S0, sol_noNoise);
